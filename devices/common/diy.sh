...
sed --follow-symlinks -i "s#%C\"#%C by ShiFufWrt\"#" package/base-files/files/etc/os-release
...
coremark wget-ssl curl autocore htop nano zram-swap kmod-lib-zstd kmod-tcp-bbr bash openssh-sftp-server block-mount resolveip ds-lite swconfig luci-app-fan luci-app-filemanager luci-app-wifihistory \
luci-app-adbyby-plus luci-app-store luci-app-ddnsto \
luci-app-passwall luci-app-openclash /" include/target.mk
...
sed -i "s/OpenWrt/ShiFufWrt/g" package/base-files/files/bin/config_generate package/base-files/image-config.in package/network/config/wifi-scripts/files/lib/wifi/mac80211.uc config/Config-images.in Config.in include/u-boot.mk include/version.mk || true
...
# WiFi 预配置：隐藏 ESSID，2.4G=24，5G=50，密码统一
mkdir -p package/base-files/files/etc/uci-defaults
cat > package/base-files/files/etc/uci-defaults/99-wifi-defaults << 'WIFI_EOF'
#!/bin/sh
# 2.4G WiFi
uci set wireless.default_radio0.ssid='24'
uci set wireless.default_radio0.encryption='sae-mixed'
uci set wireless.default_radio0.key='147258aaaAAA*'
uci set wireless.default_radio0.hidden='1'
uci set wireless.default_radio0.disabled='0'
# 5G WiFi
uci set wireless.default_radio1.ssid='50'
uci set wireless.default_radio1.encryption='sae-mixed'
uci set wireless.default_radio1.key='147258aaaAAA*'
